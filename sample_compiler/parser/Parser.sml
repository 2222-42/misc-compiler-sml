...
struct
  ...
  fun discardSemicolons stream =
      ...
      in
          if P.sameToken (token, T.SEMICOLON (0, 0)) then
              discardSemicolons rest
          else
              if P.sameToken (token, T.EOF (0,0)) then raise EOF
              else stream
      end
  fun doParse stream =
      let
          ...
          val (dec, stream) =
              P.parse {lookahead=0, stream=stream, error=print_error, arg=()}
  ...
  fun makeStream inStream =
      let
          val lexer = CoreMLLex.makeLexer (fn n => TextIO.inputN (inStream, 1))
      in
